...
//if we keep on inserting the data into the table then log file size increases...then we do backup also .trn file increases but the disk space allocated to 
//.log file doesn't change ..so then use shrink to decrease the allocated disk space 
...
--BACKUP DATABASE anirudh TO DISK ='C:\SQLBackup\anirudhFUll.bak' with name = 'test1';              //logical name can be set using with name..to see logical name run 
                                                                                                      headeronly command

--restore FILELISTONLY from disk='C:\SQLBackup\anirudhFUll.bak'                     //if we would like to restore the outer available db to a location first we need kn                        
                                                                                      know the logical name for that we use filelistonly
